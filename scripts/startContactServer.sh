...
do
	...
	then
        result="`ssh -n -f ubc_eece411_4@$host "ps -A -o pid,cmd|grep '$2' | grep -v grep |head -n 1"`"
        if [ ${#result} -eq 0 ]; then
            ssh -n -f ubc_eece411_4@$host "sh -c 'java -Xmx64m -jar DHT-Group9/server-group9.jar -n 0 -p $2 > /dev/null 2>&1 &'"
            check="`ssh -n -f ubc_eece411_4@$host "ps -A -o pid,cmd|grep '$2' | grep -v grep |head -n 1"`"
            if [ ${#check} -eq 0 ]; then
